...
public class PsqlStore implements Store {
    private static final String FIND_ALL_SEATS = "SELECT * FROM public.cinema_hall";
    private static final String FIND_ALL_ACCOUNTS = "SELECT * FROM public.account";
    private static final String SET_SEAT_OWNER = "UPDATE public.cinema_hall set account_id = ? where id = ?";
    private static final String CREATE_ACCOUNT_WITH_PASS = "INSERT into public.account (name, email, " +
            "password) values " +
            "(?, ?, crypt(?, gen_salt('bf', 8)))";
    private static final String CREATE_ACCOUNT = "INSERT into public.account (name, email) values " +
            "(?, ?)";
    ...
    private PsqlStore() {
        ...
        try (BufferedReader io = new BufferedReader(
                new FileReader("/home/anton/development/java-code/enterprise-systems-architecture/lab1/src/main/resources/db.properties")
        )) {
            cfg.load(io);
        } catch (Exception e) {
